...
void Genome::ReadFromFile(char *filename)
{
  ...
  double scale, thresh, scale1, scale2;
...
void Genome::PrintGenome(char *filename)
{
  ...
  int count;
  for (auto n: regnodes){
    //cout<<n.genenr<<", printed regnode"<<endl;
  ...
  ofs <<" "<<endl;
  for (auto n: outputnodes){
    //cout<<n.genenr<<", printed outnode"<<endl;
    ofs << n.genenr<<" [label=\""<<n.genenr<<", "<<n.threshold<<"\"];"<<endl;
    count=0;
    for (const auto w: n.w_regnode) {
      ofs << count+innr <<"-> "<<n.genenr<<"[label=\" "<<w<<"\"];"<<endl;
...
int i;
  ...
  //cout<<"input scales: "<<inputscale[0]<<inputscale[1]<<endl;
  //cout<<"regulatory nodes"<<endl;
