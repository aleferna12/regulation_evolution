...
int CellularPotts::DeltaH(int x, int y, int xp, int yp, PDE *PDEfield) {
  ...
  if (sxyp == MEDIUM) {
    ...
    DH += (int) ((par.lambda * (2. + 2. *
                                     (double) ((*cell)[sxyp].Area() - (*cell)[sxyp].TargetArea() - (*cell)[sxy].Area() +
                                               (*cell)[sxy].TargetArea()))));
            ...
            for (int j = 1; j <= n_nb; j++) {
              ...
              if (xn > 0 && yn > 0 && xn < sizex - 1 && yn < sizey - 1) {//if the neighbour site is within the lattice
                ...
                if (sigma[xn][yn] != sigma[x][y] &&
                    sgn(sigma[xn][yn]) + sgn(sigma[x][y]) >= 0) { //if we should add the edge to the edgelist, add it
...
//       cerr<<"sxy!=0, lb1: "<<lb1<<", syy: "<<syy<<", sxx: "<<sxx<<", sxy: "<<sxy<<endl;
      ...
      celldir[i].aa1 =
              ymean - xmean * celldir[i].bb1; //this is the intercept to the y axis of the line with slope first eigenv.
      ...
      celldir[i].aa2 =
              ymean - celldir[i].bb2 * xmean; // this is the intercept to y axis of the line of the second eigenvector
...
int CellularPotts::PlaceCellsOrderly(int n_cells, int size_cells) {
  ...
  cerr << "Placed " << count << " cells out of " << n_cells << " requested; avrg area = " << avrg_area / (double) count
       << endl;
  //exit(1);
  return count;
}
