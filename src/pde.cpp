...
// Plot the value of the PDE only in the medium of the CPM  <--- This is the one you want to use to store pictures!!!
...
void PDE::GradC(int layer, int first_grad_layer) {
    ...
    for (int y = 0; y < sizey; y++) {
        for (int x = 1; x < sizex - 1; x++) {
            sigma[first_grad_layer + 2][x][y] =
                    sigma[layer][x + 1][y] - sigma[layer][x - 1][y] - 2 * sigma[layer][x][y];
        }
    }
    ...
    for (int x = 0; x < sizex; x++) {
        for (int y = 1; y < sizey - 1; y++) {
            sigma[first_grad_layer + 3][x][y] =
                    sigma[layer][x][y - 1] - sigma[layer][x][y + 1] - 2 * sigma[layer][x][y];
        }
    }
}
