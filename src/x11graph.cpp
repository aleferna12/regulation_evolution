...
void X11Graphics::ReadColorTable(XColor *cols, const char *colortable_filename) {
    ...
    while (fscanf(fpc, "%d", &i) != EOF) {
        fscanf(fpc, "%d %d %d\n", &p, &q, &r);
        //cols[i].red=p*255; // isn't this wrong? might be some compatibility issue with qt stuff?
        // old version up here, fixed throughout
        cols[i].red = p;
        cols[i].green = q;
        cols[i].blue = r;
    }
...
//Apparently this function is not called, and it seems that colors are actually decided
// on the first call of Write()
void X11Graphics::MakeColorMap() {
    ...
    XQueryColors(display, DefaultColormap(display, screen), colors, colormap_size);
...
Coordinate X11Graphics::ReplaceBeast(Coordinate old_size, Coordinate new_size) {
    ...
    XGrabPointer(display, window, TRUE, ButtonPressMask | PointerMotionMask, GrabModeAsync, GrabModeAsync, window, hand,
                 CurrentTime);
